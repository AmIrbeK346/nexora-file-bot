...
# ==========================================
# 1. YORDAMCHI FUNKSIYALAR (Rename va Action)
# ==========================================
...
async def finalize_with_custom_name(message: Message, state: FSMContext, bot: Bot, l10n: dict, lang: str):
    ...
    await message.answer_document(
        FSInputFile(file_path, filename=full_name), 
        caption=l10n['done'],
        reply_markup=get_main_menu(lang)
    )
    await state.clear()
...
# ==========================================
# 3. ASOSIY AMALLAR (Merge, Office, ZIP, JPG, Organize)
# ==========================================
...
async def process_office_to_pdf(message: Message, state: FSMContext, bot: Bot, lang: str, l10n: dict):
    ...
    try:
        ...
        if out_f.exists():
            ...
            raise Exception("LibreOffice error")
...
async def finalize_jpg(message: Message, state: FSMContext, lang: str, l10n: dict):
    ...
    try:
        ...
        for img_p in files:
            p_obj = Path(img_p)
            clean_p = p_obj.with_suffix('.tmp.jpg')
            with Image.open(img_p) as img:
                rgb_img = img.convert('RGB')
                rgb_img.save(str(clean_p), "JPEG", quality=90)
...
# --- PARAMETRLAR VA PDF PROCESSOR (Organize/Security/From PDF) ---
...
async def process_pdf_unified(message: Message, state: FSMContext, bot: Bot, l10n: dict, lang: str):
    ...
    data = await state.get_data()
    tool, params = data.get('current_tool'), data.get('page_input', "")
    status = await message.answer(l10n['processing'])
    in_p = Path(TEMP_DIR).resolve() / f"pdf_in_{message.from_user.id}_{int(time.time())}.pdf"
    out_p = Path(TEMP_DIR).resolve() / f"pdf_out_{message.from_user.id}_{int(time.time())}.pdf"
    ...
    try:
        ...
        await message.answer(l10n['error_generic'].format(e=e))
        await state.clear()
    finally:
        if in_p.exists(): os.remove(str(in_p))

# --- WATERMARK & PASSWORD HANDLERS ---
...
async def get_watermark_text(message: Message, state: FSMContext, l10n: dict):
    ...
    await message.answer(l10n.get('watermark_saved', "✅ {text}").format(text=message.text))
    await state.set_state(Form.waiting_for_pdf)
